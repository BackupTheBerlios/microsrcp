...
#else   //  DCCISR_IS_OPTIMIZED

//----------------------------------------------------------------------------------------
// Standard code, no special optimizing
...
void do_send(bool myout)
  {
    if (myout == 0)
      {                               // 0 - make a long pwm pulse
        TCCR1A = (1<<COM1A1) | (0<<COM1A0)  //  clear OC1A (=DCC) on compare match
               | (1<<COM1B1) | (1<<COM1B0)  //  set   OC1B (=NDCC) on compare match
               | (0<<FOC1A)  | (0<<FOC1B)   //  reserved in PWM, set to zero, also reserved in atmega644p
               | (0<<WGM11)  | (0<<WGM10);  //  CTC (together with WGM12 and WGM13)
        OCR1A = F_CPU * PERIOD_0 / 2 / 1000000L;               //1856
        OCR1B = F_CPU * PERIOD_0 / 2 / 1000000L;               //1856
      }
    ...
    if (myout == 0)
      ...
      {                                     // 1 - make a short pwm puls
        TCCR1A = (1<<COM1A1) | (0<<COM1A0)  //  clear OC1A (=DCC) on compare match
               ...
               | (0<<WGM11)  | (0<<WGM10);  //  CTC (together with WGM12 and WGM13)
        // OCR1A = F_CPU * PERIOD_1  / 2 / 1000000L ;            //928
        OCR1A = F_CPU * CUTOUT_GAP / 1000000L ;            
        OCR1B = F_CPU / 1000000L * 8 * PERIOD_1 / 2;          // extended (cutout starts after OCR1A)
      }
  }
	    ...
	    if ((doi.state == dos_cutout_2) && doi.railcom_enabled)
          {
            TCCR1A = (1<<COM1A1) | (1<<COM1A0)  //  set   OC1A (=DCC) on compare match
                   ...
                   | (0<<FOC1A)  | (0<<FOC1B)   //  reserved in PWM, set to zero
                   | (0<<WGM11)  | (0<<WGM10);  //  CTC (together with WGM12 and WGM13)
            // OCR1A = F_CPU / 1000000L * 7 * PERIOD_1 / 2;   // create extended timing: 3.5 * PERIOD_1 for DCC
    		// OCR1B = F_CPU / 1000000L * 8 * PERIOD_1 / 2;   //                         4 * PERIOD_1 for NDCC
    		OCR1A = (F_CPU / 1000000L * 4 * PERIOD_1)
                    - (F_CPU / 1000000L * CUTOUT_GAP);    // create extended timing: 4 * PERIOD_1 for DCC - GAP
    		OCR1B = (F_CPU / 1000000L * 9 * PERIOD_1 / 2)   //                            4.5 * PERIOD_1 for NDCC - GAP
                    - (F_CPU / 1000000L * CUTOUT_GAP);
    ...
    switch (doi.state)
      {
            ...
            if (next_message_count > 0)
              {
                ...
                doi.xor_byte = 0;
                doi.type = next_message.type;      // remember type in case feedback is required
                if (PROG_TRACK_STATE) doi.bits_in_state = 20-3;    // long preamble if service mode
				else 				  doi.bits_in_state = 14-3;    // standard preamble
        ...
        case dos_end_bit:
            do_send(1);                                   // last 1
            doi.state = dos_cutout_1;
            break;

        case dos_cutout_1:
            if (doi.railcom_enabled) do_send_no_B(1);     // first 1 after message gets extended
            else do_send(1);
            doi.state = dos_cutout_2;
            break;
...
//-------------------------------------------------------------------------------------
//// --- the following code is only useful for a simulator - no timing involved

#if (DCC_SIMU)
void timer1_simu(void)
  {
    switch (doi.state)
      {
        case dos_idle:
...
//
// Diese Maschine muß mit den "aufgeblasenen" Trits gefüttert werden.
...
// Adresserweiterung gemäß Intellibox
