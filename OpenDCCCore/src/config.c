...
// Die folgende (assemblerähnliche) Defintion der EEPROM-Variablen ist
// erforderlich, um diese compilerunabhängig auf feste Adressen zu legen.
// Auf diese Adressen kann von außen mit den Befehl zum Lesen und Schreiben
...
uint8_t ee_mem[] EEMEM =
   ...
   #if (__AVR_ATmega32__)
        uint8_t ee_mem[] EEMEM =
   #elif (__AVR_ATmega644P__ || __AVR_ATmega328P__)
...
#endif
{
    ...
    [eadr_OpenDCC_Mode]             =                       // here we code the features of OpenDCC
                                      ...
                                      (0 << 6) |            // Bit 6; reserved
                                      (0 << 7),             // Bit 7; reserved
    [eadr_virtual_decoder_l]        = (unsigned char) (DMX_MACRO_ADDR),
    [eadr_virtual_decoder_h]        = (unsigned char) (DMX_MACRO_ADDR >> 8),
    ...
    [eadr_railcom_enabled]          = 0,                    // CV25 - railcom
    ...
    [eadr_short_turnoff_time]       = SHORT_TURNOFF_TIME / TICK_PERIOD,   // 34: Time until shutdown[eadr_reserved034]              = 0,          
    [eadr_prog_short_toff_time]     = PROG_SHORT_TURNOFF_TIME / TICK_PERIOD,          
    #if (TURNOUT_FEEDBACK_ACTIVATED == 1)
    ...
    #endif
    [eadr_ext_stop_deadtime]        = 6,                    // 37: dead time after RUN, in systick [5ms]          
    [eadr_reserved038]              = 0,          
