...
// 
// Gestartet wird eine Aufgabe durch setzen der Variablen und anschließend
...
#define TIME_REMEMBER_BIT_OP  500000L       // after 500ms OpenDCC forgets about the abilitiy
                                            // of the decoder to do bit operations
signed char last_bit_check;

int page_loaded_in_decoder = -1;            // 

#define TIME_REMEMBER_PAGE    500000L       // after 500ms OpenDCC forgets the loaded page addr
                                            
signed char last_page_loaded;

#if (TIME_REMEMBER_BIT_OP / TICK_PERIOD > 127)
  #warning TIME_REMEMBER_BIT_OP too large
#endif
...
t_prog_ctrl direct_ctrl_default PROGMEM       = {P_READ, {3, 0, 0, 5,  6}};
t_prog_ctrl registermode_ctrl_default PROGMEM = {P_READ, {3, 5, 9, 7, 10}};
...
volatile signed char timerval;          // is incremented every 5th timertick
volatile signed char micro_timerval;    // is incremented every timertick, wrap around

///-------------------------------------  This interrupt ticks every 5ms

#if (__AVR_ATmega644P__ || __AVR_ATmega328P__ || __AVR_ATmega1280__)
    ...
    ISR(TIMER0_COMP_vect)
      {
        OCR0 +=  F_CPU / 64L * MY_TICK_PERIOD / 1000000L;
        micro_timerval++;
        if (micro_timerval == 5)
          {
            micro_timerval = 0;
    ...
    switch(opendcc_state_before_prog)
      {
        ...
        case RUN_SHORT:            // Kurzschluß
        case RUN_PAUSE:            // DCC Running, all Engines Speed 0
        	opendcc_state = opendcc_state_before_prog;
            break;
...
void run_programmer(void)
  {
    if ((signed char)(timerval - last_bit_check) > (TIME_REMEMBER_BIT_OP / TICK_PERIOD))
      {
        decoder_can_bit_operations = 0;                     // ist ab jetzt void
        last_bit_check = timerval;
      }

    if ((signed char)(timerval - last_page_loaded) > (TIME_REMEMBER_PAGE / TICK_PERIOD))
      {
        page_loaded_in_decoder = -1;                     // ist ab jetzt void
        last_page_loaded = timerval;
      }
    ...
    switch (prog_seq_state)
      {
        ...
        case PS_START:
            ...
            switch (ps_command)
              {
                ...
                case PSC_DCCRL:                     // read long adr
                    pb_command = PBC_CVM_R_BIT; // now cv17
                    prog_result_size = 2;
            ...
            break;
        case PS_RUNNING:
            prog_result = pb_result;      // einfach durchreichen !!!!
            ...
            if (pb_result == PT_OKAY)
              {
                ...
                last_page_loaded = timerval;        // save timestamp and page;
...
void init_programmer(void)
  {
    ...
    //i = eeprom_read_byte((void *)eadr_extend_prog_resets);
    /*if (i > 10)*/ i= 10;  // limit
    direct_ctrl.cycles[0] += i;
    registermode_ctrl.cycles[0] += i;
    
    //i = eeprom_read_byte((void *)eadr_extend_prog_command);
    /*if (i > 10)*/ i= 10;  // limit
...
/*unsigned char my_XPT_DCCRD (unsigned int cv)
  {
    ...
    if (key_go_at_start_pressed)
      {
        prog_data = eeprom_read_byte((void *)(eadr_OpenDCC_Version + cv));
        prog_result = PT_OKAY;
        prog_result_size = 1;
        prog_event.result = 1;
        return(0);
...
/*unsigned char my_XPT_DCCWD (unsigned int cv, unsigned char data)
  {
    ...
    if (key_go_at_start_pressed)
      {
        eeprom_write_byte((void *)(eadr_OpenDCC_Version + cv), prog_data);
        prog_result = PT_OKAY;
        prog_result_size = 0;
        prog_event.result = 1;
        return(0);
