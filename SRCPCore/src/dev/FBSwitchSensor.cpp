...
FBSwitchSensor::FBSwitchSensor( int addr, uint8_t startPin, uint8_t endPin, srcp::SRCPFeedback* next )
{
	this->addr = addr;
...
{
	// alle Werte abfragen, nur der Wechsel auf HIGH fuehrt zu Aenderung des Sensorstatuses
	for	( int i = startPin; i <= endPin; i++ )
	{
